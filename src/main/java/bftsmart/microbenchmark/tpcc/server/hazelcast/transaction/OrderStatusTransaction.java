...
public class OrderStatusTransaction implements Transaction {
    ...
    public TransactionResponse process(final TransactionRequest command) {
        OrderStatusRequest input = (OrderStatusRequest) command;
        OrderStatusResponse orderStatusOutput = new OrderStatusResponse().withDateTime(LocalDateTime.now());
        ...
        if (order != null) {
            ...
            Collection<OrderLine> orderLines = orderLineRepository.find(order.getOrderId(), districtId, warehouseId);

            OrderLineResponse[] orderLineOutputs = new OrderLineResponse[orderLines.size()];
            for (int i = 0; i < orderLines.size(); i++) {
                OrderLine orderLine = IterableUtils.get(orderLines, i);
                orderLineOutputs[i] = new OrderLineResponse().withSupplierWarehouseId(orderLine.getSupplyWarehouseId())
        ...
        return new TransactionResponse().withStatus(0).withResponse(outputScreen(orderStatusOutput));
    }

    private String outputScreen(OrderStatusResponse orderStatus) {
        ...
        if (orderStatus.getOrderId() == 0) {
            ...
            if (ArrayUtils.isNotEmpty(orderStatus.getOrderLines())) {
                ...
                for (OrderLineResponse oLine : orderStatus.getOrderLines()) {
                    message.append("[");
